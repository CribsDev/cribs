...
enum WalletFeature {
    ...
    FEATURE_PRE_CRIBX = 61000, // inherited version..
...
// Possible states for zCRIB send
enum ZerocoinSpendStatus {
    ZCRIB_SPEND_OKAY = 0,                            // No error
    ZCRIB_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCRIB_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCRIB_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCRIB_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCRIB_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCRIB_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCRIB_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCRIB_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCRIB_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZCRIB_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCRIB_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCRIB_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCRIB_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCRIB_SPENT_USED_ZCRIB = 14,                      // Coin has already been spend
    ZCRIB_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZCRIB_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 CRIB output and keys which can be used for the Masternode
    ...
    void doZPivRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZCRIB);
    ...
    bool CreateZCRIBOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zCRIB wallet
    CzCRIBWallet* zwalletMain{nullptr};
    std::unique_ptr<CzCRIBTracker> zpivTracker{nullptr};
    void setZWallet(CzCRIBWallet* zwallet);
    CzCRIBWallet* getZWallet();
    ...
    // zCRIB reset
    boost::signals2::signal<void()> NotifyzCRIBReset;
