...
#if defined(NDEBUG)
#error "CRIBX cannot be compiled without assertions."
#endif
        ...
        bool* pfMissingInputs, bool fOverrideMempoolLimit, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zCRIB mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zCRIB mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 1 	&& nHeight <= 14400) 		return 0.1 * COIN;
    if (nHeight > 14401 && nHeight <= 21601) 		return 0.15 * COIN;
    if (nHeight > 21602 && nHeight <= 28802) 		return 0.2 * COIN;
    if (nHeight > 28803 && nHeight <= 36003) 		return 0.3 * COIN;
    if (nHeight > 36004 && nHeight <= 43204) 		return 0.5 * COIN;
    if (nHeight > 43205 && nHeight <= 57605) 		return 0.8 * COIN;
    if (nHeight > 57606 && nHeight <= 72006) 		return 1 * COIN;
    if (nHeight > 72007 && nHeight <= 86407) 		return 1.1 * COIN;
    if (nHeight > 86408 && nHeight <= 100808) 		return 1.2 * COIN;
    if (nHeight > 100809 && nHeight <= 115209) 		return 1.3 * COIN;
    if (nHeight > 115210 && nHeight <= 129610) 		return 1.4 * COIN;
    if (nHeight > 129611 && nHeight <= 144011) 		return 1.5 * COIN;
    if (nHeight > 144012 && nHeight <= 158412) 		return 1.6 * COIN;
    if (nHeight > 158413 && nHeight <= 172813) 		return 1.7 * COIN;
    if (nHeight > 172814 && nHeight <= 187214) 		return 1.8 * COIN;
    if (nHeight > 187215 && nHeight <= 201615) 		return 1.9 * COIN;
    if (nHeight > 201616 && nHeight <= 216016) 		return 2 * COIN;
    if (nHeight > 216017 && nHeight <= 230417) 		return 2.1 * COIN;
    if (nHeight > 230418 && nHeight <= 403218) 		return 2.2 * COIN;
    if (nHeight > 403219 && nHeight <= 748819) 		return 2.3 * COIN;
    if (nHeight > 748820 && nHeight <= 1094420) 	return 2.4 * COIN;
    if (nHeight > 1094421 && nHeight <= 1440021) 	return 2.5 * COIN;
    if (nHeight > 1440022 && nHeight <= 1785622) 	return 2.6 * COIN;
    if (nHeight > 1785623 && nHeight <= 2131223) 	return 2.7 * COIN;
    if (nHeight > 2131224 && nHeight <= 2476824) 	return 2.8 * COIN;
    if (nHeight > 2476825 && nHeight <= 2822425) 	return 2.9 * COIN;
    if (nHeight > 2822426 && nHeight <= 3300506) 	return 3 * COIN;
    if (nHeight > 3300507 && nHeight <= 3818907) 	return 3.1 * COIN;
...
{
    if (nHeight == 1) return 0 * COIN;
    if (nHeight > 1 	&& nHeight <= 14400) 		return 0.08 * COIN;
    if (nHeight > 14401 && nHeight <= 21601) 		return 0.12 * COIN;
    if (nHeight > 21602 && nHeight <= 28802) 		return 0.16 * COIN;
    if (nHeight > 28803 && nHeight <= 36003) 		return 0.24 * COIN;
    if (nHeight > 36004 && nHeight <= 43204) 		return 0.38 * COIN;
    if (nHeight > 43205 && nHeight <= 57605) 		return 0.60 * COIN;
    if (nHeight > 57606 && nHeight <= 72006) 		return 0.70 * COIN;
    if (nHeight > 72007 && nHeight <= 86407) 		return 0.77 * COIN;
    if (nHeight > 86408 && nHeight <= 100808) 		return 0.78 * COIN;
    if (nHeight > 100809 && nHeight <= 115209) 		return 0.85 * COIN;
    if (nHeight > 115210 && nHeight <= 129610) 		return 0.91 * COIN;
    if (nHeight > 129611 && nHeight <= 144011) 		return 0.98 * COIN;
    if (nHeight > 144012 && nHeight <= 158412) 		return 1.04 * COIN;
    if (nHeight > 158413 && nHeight <= 172813) 		return 1.11 * COIN;
    if (nHeight > 172814 && nHeight <= 187214) 		return 1.17 * COIN;
    if (nHeight > 187215 && nHeight <= 201615) 		return 1.24 * COIN;
    if (nHeight > 201616 && nHeight <= 216016) 		return 1.30 * COIN;
    if (nHeight > 216017 && nHeight <= 230417) 		return 1.37 * COIN;
    if (nHeight > 230418 && nHeight <= 403218) 		return 1.54 * COIN;
    if (nHeight > 403219 && nHeight <= 748819) 		return 1.73 * COIN;
    if (nHeight > 748820 && nHeight <= 1094420) 	return 1.92 * COIN;
    if (nHeight > 1094421 && nHeight <= 1440021) 	return 2 * COIN;
    if (nHeight > 1440022 && nHeight <= 1785622) 	return 2.08 * COIN;
    if (nHeight > 1785623 && nHeight <= 2131223) 	return 2.16 * COIN;
    if (nHeight > 2131224 && nHeight <= 2476824) 	return 2.24 * COIN;
    if (nHeight > 2476825 && nHeight <= 2822425)	return 2.32 * COIN;
    if (nHeight > 2822426 && nHeight <= 3300506)	return 2.40 * COIN;
    if (nHeight > 3300507 && nHeight <= 3818907) 	return 2.48 * COIN;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZCRIBModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zCRIB money supply
    if (!UpdateZCRIBSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zCRIB supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("cribs-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZCRIBModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    // Update zCRIB money supply map
    if (!UpdateZCRIBSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zCRIB supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zCRIB supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateCRIBSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCRIB spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCRIB serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Cribs Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nCribsBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nCribsBadBlockBits)) {
            // accept CRIBX block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        std::vector<CTxIn> zCRIBInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zCRIBInputs.push_back(stakeIn);
            }else{
                pivInputs.push_back(stakeIn);
            }
        }
        const bool hasCRIBInputs = !pivInputs.empty();
        const bool hasZCRIBInputs = !zCRIBInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZCRIBInputs && in.IsZerocoinSpend())
            ...
            // Now that this loop if completed. Check if we have zCRIB inputs.
            if(hasZCRIBInputs) {
                for (const CTxIn& zPivInput : zCRIBInputs) {
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // CRIBX: We use certain sporks during IBD, so check to see if they are
