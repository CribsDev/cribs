...
*Setup instructions for a gitian build of CRIBX Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the CRIBX
...
to the CRIBX GitHub release page.
...
- [Building CRIBX Core](#building-cribs-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cribs-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cribs-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cribs-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cribs-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cribs-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cribs-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/cribs-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@cribs.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cribs-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cribs-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cribs-osx-*-build.assert
```
