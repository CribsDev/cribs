...
std::string ReindexZerocoinDB()
{
    ...
    while (pindex) {
        ...
        // update supply
        UpdateZCRIBSupplyConnect(block, pindex, true);
...
{
    CDataStream serializedCoinSpend = ZCRIBModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZCRIBSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zCRIB supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zCRIB supply
    ...
    // A one-time event where only the zCRIB supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZCRIBSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zCRIB supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zCRIB supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
