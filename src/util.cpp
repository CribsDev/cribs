...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CRIBX
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CRIBX
// Mac: ~/Library/Application Support/CRIBX
// Unix: ~/.cribs
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CRIBS";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "cribs.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cribs.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cribs.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "cribsd.pid"));
