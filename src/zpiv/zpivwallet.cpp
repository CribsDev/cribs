...
CzCRIBWallet::CzCRIBWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCRIBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCRIBSeed_deprecated()) {
                    LogPrintf("%s: Updated zCRIB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzCRIBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCRIBCount(nCountLastUsed);
    else if (!walletdb.ReadZCRIBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCRIBWallet::Lock()
{
    seedMaster.SetNull();
}

void CzCRIBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCRIBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCRIB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCRIBWallet::LoadMintPoolFromDB()
{
...
void CzCRIBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCRIBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCRIBWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCRIBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCRIB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzCRIBWallet::SeedToZCRIB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzCRIBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCRIBWallet::GenerateDeterministicZCRIB(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCRIBWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCRIBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCRIBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
