...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    util::ThreadRename("cribs-wallet");
...
bool CWalletDB::WriteZCRIBSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZCRIBSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZCRIBSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZCRIBSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZCRIBSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZCRIBSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZCRIBCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZCRIBCount(uint32_t& nCount)
{
