...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    pblock->nVersion = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_NEW_ALGO) ? CBlockHeader::CURRENT_VERSION : 7;
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zCRIB spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zCRIB spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zCRIB serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("CRIBSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("CRIBSMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("CRIBSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("cribs-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running CRIBXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("CRIBSMiner exception");
    } catch (...) {
        LogPrintf("CRIBSMiner exception");
    }

    LogPrintf("CRIBSMiner exiting\n");
}
