...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::CRIB);
    ...
    if (settings.contains("nAnonymizeCribsAmount") || reset)
        SoftSetArg("-anonymizecribsamount", settings.value("nAnonymizeCribsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cribs.cpp during shotdown to also remove the window positions
