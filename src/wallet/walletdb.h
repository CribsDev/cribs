...
public:
    ...
    bool WriteZCRIBSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZCRIBSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZCRIBSeed_deprecated(uint256& seed);
    bool EraseZCRIBSeed();
    bool EraseZCRIBSeed_deprecated();

    bool WriteZCRIBCount(const uint32_t& nCount);
    bool ReadZCRIBCount(uint32_t& nCount);
