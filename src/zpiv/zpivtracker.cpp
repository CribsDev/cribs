...
CzCRIBTracker::CzCRIBTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzCRIBTracker::~CzCRIBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCRIBTracker::Init()
{
...
bool CzCRIBTracker::Archive(CMintMeta& meta)
{
...
bool CzCRIBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCRIBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCRIBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCRIBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCRIBTracker::GetSerialHashes()
{
...
CAmount CzCRIBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCRIBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCRIBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCRIBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCRIBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCRIBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCRIBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCRIBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCRIBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCRIBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzCRIBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCRIBWallet* zCRIBWallet)
{
    bool iszCRIBWalletInitialized = (nullptr != zCRIBWallet);
    CMintMeta meta;
    ...
    if (!iszCRIBWalletInitialized)
        zCRIBWallet = new CzCRIBWallet(wallet);
    meta.isSeedCorrect = zCRIBWallet->CheckSeed(dMint);
    if (!iszCRIBWalletInitialized)
        delete zCRIBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCRIBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCRIBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCRIBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCRIBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCRIBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCRIBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
