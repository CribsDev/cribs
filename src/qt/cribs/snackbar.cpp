...
#include "qt/cribs/forms/ui_snackbar.h"
#include "qt/cribs/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(CRIBXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &CRIBXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
