...
#include "qt/cribs/forms/ui_dashboardwidget.h"
#include "qt/cribs/sendconfirmdialog.h"
#include "qt/cribs/txrow.h"
...
DashboardWidget::DashboardWidget(CRIBXGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &CRIBXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &CRIBXGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &CRIBXGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("CRIB")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCRIB;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair CRIB, zCRIB
